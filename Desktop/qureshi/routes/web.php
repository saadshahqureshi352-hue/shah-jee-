...
Route::middleware(['auth'])->group(function () {
    ...
    Route::prefix('api/admin')->name('api.admin.')->group(function () {
        ...
        // Dashboard data with date filtering
        Route::get('/dashboard/data', [AdminController::class, 'getDashboardData']);

        // Courier actions
        Route::post('/courier/add', [AdminController::class, 'addCourier']);

        // Merchant actions
        Route::post('/merchant/status', [AdminController::class, 'updateMerchantStatus']);
        Route::post('/merchant/edit-payment', [AdminController::class, 'editMerchantPayment']);
        Route::post('/merchant/custom-return-charge', [AdminController::class, 'saveMerchantCustomReturnCharge']);

        // Invoice actions
        Route::post('/invoice/today-pay', [AdminController::class, 'todayPay']);
        Route::get('/invoices/{invoice}/orders', [AdminController::class, 'getInvoiceOrders']);
        Route::post('/invoices/{invoice}/edit', [AdminController::class, 'editInvoice']);

        // Settlements
        Route::get('/settlements/pending', [AdminController::class, 'getPendingSettlements']);
    });
