    ...
    public static function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                ...
                Section::make('Delivery & Return Charges')
                    ->columns(3)
                    ->schema([
                        TextInput::make('different_city_delivery')
                            ->label('Different City Delivery (PKR)')
                            ->numeric()
                            ->default(240)
                            ->required()
                            ->prefix('PKR'),
                        TextInput::make('same_city_delivery')
                            ->label('Same City Delivery (PKR)')
                            ->numeric()
                            ->default(200)
                            ->required()
                            ->prefix('PKR'),
                        TextInput::make('return_charge')
                            ->label('Return Charge (PKR)')
                            ->numeric()
                            ->default(200)
                            ->required()
                            ->prefix('PKR'),
                        TextInput::make('additional_kg_rate')
                            ->label('Additional Weight per KG (PKR)')
                            ->numeric()
                            ->default(140)
                            ->required()
                            ->prefix('PKR'),
        ...
        return $table
            ->columns([
                TextColumn::make('name')
                    ->searchable()
                    ->sortable(),
                TextColumn::make('different_city_delivery')
                    ->label('Diff City')
                    ->formatStateUsing(fn ($state) => 'PKR ' . number_format($state, 0))
                    ->sortable(),
                TextColumn::make('same_city_delivery')
                    ->label('Same City')
                    ->formatStateUsing(fn ($state) => 'PKR ' . number_format($state, 0))
                    ->sortable(),
                TextColumn::make('return_charge')
                    ->label('Return')
                    ->formatStateUsing(fn ($state) => 'PKR ' . number_format($state, 0))
                    ->sortable(),
                TextColumn::make('additional_kg_rate')
                    ->label('Addl KG')
                    ->formatStateUsing(fn ($state) => 'PKR ' . number_format($state, 0))
                    ->sortable(),
