...
<x-app-layout>
    ...
    <div class="space-y-4 p-4 sm:p-6" x-data="{
        ...
        // Bulk Print Modal Extended States (Picture 2)
        showBulkModal: false,
        bulkOrders: [],
        selectedOrders: [],
        bulkDateRange: 'today',
        ...
        fetchBulkOrders() {
            ...
            fetch(`/api/bulk-print-orders?date_range=${this.bulkDateRange}`)
                .then(res => res.json())
                .then(data => {
        ...
        // Real-time Working Client side Search Filter
        get filteredOrders() {
            if (!this.bulkSearch.trim()) return this.bulkOrders;
            let term = this.bulkSearch.toLowerCase();
            return this.bulkOrders.filter(order => {
                return (order.tracking_number && order.tracking_number.toLowerCase().includes(term)) ||
                       (order.customer_name && order.customer_name.toLowerCase().includes(term)) ||
                       (order.reference_no && order.reference_no.toLowerCase().includes(term)) ||
                       (order.destination_city && order.destination_city.toLowerCase().includes(term));
            });
        },

        toggleAll() {
            if (this.selectedOrders.length === this.filteredOrders.length && this.filteredOrders.length > 0) {
                this.selectedOrders = [];
            } else {
        ...
        <div class="flex flex-wrap gap-2" x-data="{ showCourierModal: false }">
            ...
            <button type="button" class="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm">Bulk Booking</button>
            ...
            {{-- 100% WORKING EDIT: PICTURE 2 EXACT BULK PRINT MODAL --}}
            <div x-show="showBulkModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
                <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-xs" @click="showBulkModal = false"></div>
                
                {{-- Modal Wrapper --}}
                <div class="relative w-full max-w-4xl rounded-xl border border-slate-200 bg-white shadow-2xl overflow-hidden animate-in fade-in duration-150">
                    ...
                    <div class="flex items-center gap-2 border-b border-slate-200 bg-white p-2">
                        ...
                        {{-- Dynamic Client Search Bar Input --}}
                        <div class="flex-1">
                            <input type="text" x-model="bulkSearch" placeholder="Search track #, reference, or customer name..." class="w-full rounded border border-slate-300 px-3 py-1.5 text-xs focus:border-orange-500 focus:ring-orange-500">
                        </div>
